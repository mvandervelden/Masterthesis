...
\todo[inline]{OVERDENKSEL: IK DENK DAT BOIMAN'S PUNT OVER QUANTIZATION ERROR ERG BELANGRIJK IS OM NADRUK TE LEGGEN (als je kritisch kijkt: waarom is dense samplen beter dan goede punten uitkiezen?, want wel veel trager, Chum original is ook met POI en edges). Lijkt me ook belangrijk als ik probeer te denken wat Leo Dorst ervan zal vinden. Lijkt me erg nuttig om door te rekenen/redeneren wat het effect van dense sampling en de NB-assumption is op de exemplars, hypotheses en detections}

\todo{I rewrote the first bit of the introduction to comply with the remarks}
...
In this thesis I will explore the possibilities of extending the NBNN method from image classification to object detection. Some preliminary research in this direction has been done very recently\cite{becker2012codebook}, which will be used as a starting point. Following Becker \emph{et al.}\cite{becker2012codebook}, I combine NBNN based object-to-class distance estimation with exemplar-based object detection\cite{chum2007exemplar}. However, where Becker \emph{et al.} show their method works, I will investigate what it means mathematically to make the combination of NBNN with exemplar-based detection. Furthermore, I will compare results of NBNN on this task with Behmo's optimal-NBNN\cite{behmo2010towards} and test performance on the popular VOC detection data set \todo{cite, also I think the end of the paragraph is not very beautiful, should be revised later on}
...
\end{description}
Therefore, raw pixel values might not be a very good choice as description on its own.\todo[inline]{connect this with other descriptors, be shorter than now, focus on sift as that's what I'll use, give SIFT's advantages and limitations. Meaning TODO: revision of the following}
...
\todo{TODO revision of section, perhaps move NN to NBNN section, more examples of different methods, maybe combine this and next into related work on detection/classifcation?}
...
\todo[inline]{Better be incorporated in the previous subsections, otherwise it will be too much repetition...}
% section related_work (end)

\section{Naive Bayes Nearest Neighbor} % (fold)
\label{sec:naive_bayes_nearest_neighbor}
\todo[inline]{perhaps add a small section on NN (and NB?) to make the connection}
...
\section{Object Detection Using Exemplar-based Models} % (fold)
\label{sec:object_detection}
\todo[inline]{Write Theory on exemplar-Based Modeling. Focus on ISM (Leibe), Becker/ Chum, Felzenszwalb, perhaps roots of the method? Show images of the idea (perhaps comparing the methods)}
...
\section{Use of NBNN in an exemplar-Based Model} % (fold)
\label{sec:linking}
...
\label{sub:detection}
NBNN-based-detection is tested on the VOC2012 \todo{and VOC2006/7? misschien goed voor vergelijkingen} data set. The performance of detection is measured both with and without optimization of the class-specific parameters defined by \cite{behmo2010towards}. A part of the training set was set apart as a optimization set, the rest was used as labeled examples. The full validation set was used for masuring performance.

SIFT descriptors were densely sampled with a spacing of 6\todo{3 would be ideal, try if that might work)} pixels and on 4 scales (1.33, 2.0, 3.0 and 4.5 \todo{what? sigma, lookup}) from all example images, just like \cite{mccan2011local} proposes in their setup. 
