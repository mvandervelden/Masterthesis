...
Using the main points of no quantization and image-to-class distances, $k$-nearest neighbor can be performed for individual features, per class. Note that in this way, it is impossible to use $k$NN as a classifier, because no discrimination can be made between the classes\JanTodo{Explain, or remove because it is possible}. It is however useful to be able to compare the distances of a feature to the nearest neighbors in each class. this is used as the input of a Naive Bayes classifier.
...
Even though the algorithm of Boiman \emph{et al.} \cite{boiman2008defense} is very simple and requires no training phase, it does have scalability issues because of its dependence on as many individual descriptors as possible. Because all densely computed descriptors for each image in the training set have to be stored, and the nearest neighbor for each descriptor of each query image on each class has to be found, the memory usage is much higher than for example BoF methods, which use a more compact representation of images. Calculating NN can be sped up by using sophisticated approximations of the Nearest Neighbor algorithm, such as \JanTodo{cite FLANN, ANN, add some brief details about FLANN in a separate paragraph, perhaps better in the Experiments section}. The memory issue remains however.

\JanTodo{Behmo is less important now, so less detail!}
...
\begin{figure}[hbt]
    ...
    \caption(Insert illustration of the idea here)
\end{figure}
...
\begin{figure}[hbt]
    \centering
    \missingfigure[figwidth=0.8\textwidth]{Image showing the idea of LNBNN}
\end{figure}
