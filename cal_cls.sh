...
TMPFOLDER=$2
mkdir "$TMPFOLDER"
python cal_cls.py $CFGFILE $TMPFOLDER
...
# Using 4 parallel processes, define the number of iterations per batch to cover
...
for B in `seq 1 $NO_BATCHES`; do
    ...
    for SZ in $IT_SIZES; do
        ...
        for P in `seq $START_CLS $STOP_CLS`; do
            ...
            python cal_cls_test.py $CFGFILE $TMPFOLDER $B $CLS&
        done
        wait
...
python cal_cls_res.py $CFGFILE $TMPFOLDER